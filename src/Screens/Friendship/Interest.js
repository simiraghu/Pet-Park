...
const Interest = () => {
  ...
  useEffect(() => {
    if (params?.breed_arr?.length) {
      const updatedList = params.breed_arr.map(item => ({
        ...item,
        status: params?.breed?.includes(item.breed_id) || false,
      }));
      setBreedList(updatedList);
    }

    if (params?.breed?.length) {
      setSelectedBreeds(params.breed);
    }
  }, [params?.breed_arr, params?.breed]);

  consolepro.consolelog('Selected Breed -======>>', selectedBreeds);
  consolepro.consolelog('Params Selected Breed======>>', params?.breed);
            ...
            {breedList.map((item, index) => (
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  const breedIndex = updatedBreeds.indexOf(item.breed_id);
                  const newList = [...breedList];

                  if (breedIndex === -1) {
                    updatedBreeds.push(item.breed_id);
          ...
          <CommonButton
            ...
            onPress={() => {
              const selectedItems = breedList.filter(item =>
                selectedBreeds.includes(item.breed_id),
              );
              selectedItems.forEach(item => (item.status = true));

              if (
                params?.handleSelect &&
                typeof params.handleSelect === 'function'
              ) {
                params.handleSelect(selectedBreeds, selectedItems);
              } else {
                console.warn('handleSelect is not passed or not a function');
              }

              if (params?.onSelectBreeds) {
                params.onSelectBreeds(selectedBreeds);
              }
